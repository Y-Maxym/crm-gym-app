...
import com.gym.crm.app.exception.EntityIdException;
...
public class EntityExceptionHelper {
    ...
    public void checkId(Long id) {
        if (isNull(id)) {
            throw new EntityIdException(messageHelper.getMessage(ERROR_ENTITY_ID_CANNOT_BE_NULL));
        }

        if (id <= 0) {
            throw new EntityIdException(messageHelper.getMessage(ERROR_ENTITY_ID_CANNOT_BE_ZERO));
        }
    }

    public void checkEntity(Object entity) {
        if (isNull(entity)) {
            throw new EntityException(messageHelper.getMessage(ERROR_ENTITY_CANNOT_BE_NULL));
        }
    }
}
